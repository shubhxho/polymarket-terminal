...
const INTERVALS = new Set<HistoryInterval>(["1h", "6h", "1d", "1w", "1m", "max"]);
...
export async function GET(req: Request) {
  ...
  const interval: HistoryInterval = raw && INTERVALS.has(raw) ? raw : "1d";
  if (tokens.length === 0) return ok([], 0);
  try {
    // One slow token shouldn't blank the whole chart — settle and drop failures.
    const settled = await Promise.allSettled(tokens.map((t) => fetchHistory(t, interval)));
